function globalContexts = efficientGetGlobalContexts(curvatureImage,keypoints)
...
curvatureImage= imgaussfilt(secondLevelApproximation,3);

globalContexts = zeros(size(keypoints,2) * 5,12);
for keypointId = 1:size(keypoints,2)
    keypointX =keypoints(1,keypointId)/4;
    keypointY =keypoints(2,keypointId)/4;
    keypointSize =keypoints(3,keypointId)/4;
